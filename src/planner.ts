 ...
 */
export function profileRisk(data: PrData): RiskProfile {
  ...
  if (changedLines > 400) riskSignals.push('large-change')
  if (files.length > 10) riskSignals.push('wide-change')
 ...
 */
export function planReview(data: PrData, adaptive: boolean, configBatchSize: number): ReviewPlan {
  if (!adaptive) return fixedPlan(configBatchSize, 'adaptive-off')
  const profile = profileRisk(data)
...
export function renderPlanSection(plan: ReviewPlan, actualTokens: number): readonly string[] {
